...
data class Instrument(
    val id: Int? = null,  // id is auto-generated in database, so it's nullable
    val name: String
)
...
fun addInstrument(name: String) {
    CoroutineScope(Dispatchers.IO).launch {
        try {
            Log.d("Supabase", "Dodawanie instrumentu: $name") // Debugowanie

            val response = supabase.from("instruments").insert(Instrument(name = name))

            Log.d("Supabase", "Dodano instrument: $response") // Sprawdzenie odpowiedzi
        } catch (e: Exception) {
...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnAddInstrument.setOnClickListener {
            addInstrument("Guitar") // Możesz podmienić na dynamiczne dodawanie nazwy
        }
    }
}