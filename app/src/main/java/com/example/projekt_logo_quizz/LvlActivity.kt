...
class LvlActivity : AppCompatActivity() {
    ...
    private fun setupGame(correctAnswer: String) {
        ...
        hintRevealedSlots = MutableList(answerSlots.size) { false }
    ...
    private fun onAnswerSlotClicked(textView: TextView) {
        val index = answerSlots.indexOf(textView)
        if (index != -1 && !hintRevealedSlots[index] && textView.text != "_" && textView.text != " ") {
